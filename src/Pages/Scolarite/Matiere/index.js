...
const CrudTable = () => {
  ...
  const fetchData = async () => {
    ...
    try {
        const response = await axiosInstance.get('/api/matiere', {
            params: {
  ...
  const handleMenuClick = (record, action) => {
    ...
    if (action === 'edit') {
      ...
      showDeleteConfirm(record.ID_Matiere);
    } else if (action === 'view') {
      setDrawerType('view');
      setDrawerVisible(true);
    }
  ...
  const showDeleteConfirm = (id) => {
    Modal.confirm({
      title: 'Êtes-vous sûr de vouloir supprimer cette matiere?',
      okText: 'Oui',
      okType: 'danger',
      cancelText: 'Non',
      onOk() {
  ...
  const handleDelete = async (id) => {
    try {
      await axiosInstance.delete(`/api/matiere/${id}`);
      message.success('Matiere supprimé avec succès');
      fetchData();
    } catch (error) {
        message.error('Impossible de supprimer la matiere. Il existe des enregistrements associés.');
    }
  };
  ...
  const columns = [
   ...
   
    {
        ...
        title: <Text strong style={{ fontSize: '16px' }}>Nom matiere</Text>,
   ...
   
    {
      ...
      title: <Text strong style={{ fontSize: '16px' }}>Nom classe</Text>,
      dataIndex: 'NomClasse',
      key: 'NomClasse',
  ...
  const handleFormSubmit = async (values) => {
    try {
      ...
      if (drawerType === 'add') {
        await axiosInstance.post('/api/matiere', values);
        message.success('Matiere ajouté avec succès');
      } else if (drawerType === 'edit' && selectedRecord) {
        const updatedValues = { ...selectedRecord, ...values }; // Ensure ID is included
        await axiosInstance.put(`/api/matiere/${selectedRecord.ID_Matiere}`, updatedValues);
        message.success('Matiere modifié avec succès');
      }
  ...
  useEffect(() => {
    const fetchFormateurOptions = async () => {
      try {
        const response = await axiosInstance.get('/api/formateurs');
        setFormateurOptions(response.data);
      } catch (error) {
        console.error('Error fetching filiere options:', error);
        message.error('Erreur lors du chargement des options de classe');
      }
...
<Form.Item
      ...
      >
        ...
        >
          {formateurOptions.map(formateur => (
            <Option key={formateur.ID_Formateur} value={formateur.ID_Formateur} style={{ fontSize: '16px' }}>
    ...
    <div style={{ padding: '40px', fontSize: '16px' }}>
      <Card style={{ borderRadius: '10px', border: '1px solid rgba(0, 0, 0, 0.1)', boxShadow: '2px 6px 14px rgba(0, 0, 0.1, 0.2)' }}>
        <Space direction="vertical" style={{ width: '100%' }}>
          <Title level={3} style={{ fontSize: '24px' }}>Liste des matières</Title>
          ...
          <Table columns={columns} dataSource={data} rowKey="ID_Matiere" pagination={pagination} loading={refreshLoading}
 ...
 {drawerType === 'add' ? 'Ajouter matiere' : drawerType === 'edit' ? 'Modifier matiere' : 'Afficher matiere'}
...
>
 ...
 <Descriptions.Item label={<Text strong style={{ fontSize: '16px' }}>Nom matière</Text>}>
